...
void loop() {
  ...
  for (int i = 0; i < 20; i++) {
      ...
      for (int j = 0; j < sizeof(legs)/sizeof(LegPos); j++) {
        if (j > 3) {
          servoWrite(legs[j].upper, record[i].upper, spd);
          servoWrite(legs[j].middle, record[i].middle, spd);
          servoWrite(legs[j].lower, record[i].lower, spd);
        } else {
          servoWrite(legs[j].upper, 1024 - record[i].upper, spd);
          servoWrite(legs[j].middle, 1024 - record[i].middle, spd);
          servoWrite(legs[j].lower, 1024 - record[i].lower, spd);
        }
